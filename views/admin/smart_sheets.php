...
<div class="space-y-6" x-data="{ 
                ...
                <div class="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between space-y-4 hover:border-emerald-300 transition group">
                    <div>
                        <div class="flex items-start justify-between gap-2">
                            <h3 class="font-extrabold text-slate-900 text-sm group-hover:text-emerald-700 transition"><?= htmlspecialchars($sTitle) ?></h3>
                        ...
                        <div class="text-xs text-slate-500 mt-3 space-y-1.5 bg-slate-50 p-3 rounded-2xl border border-slate-100 font-medium">
                            <div class="flex items-center justify-between">
                                ...
                                <strong class="text-slate-800 font-mono"><?= $rowCnt ?></strong>
                            </div>
                            <div class="flex items-center justify-between">
                                <span>Imported by:</span>
                                <strong class="text-slate-800"><?= htmlspecialchars($s['uploader_name'] ?? 'HR') ?></strong>
                            </div>
                    ...
                    <div class="pt-3 border-t border-slate-100 flex items-center justify-between">
                        <button type="button" @click="openSheetViewer(<?= htmlspecialchars(json_encode($s)) ?>)" class="text-xs font-extrabold text-emerald-600 hover:text-emerald-700 flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 transition cursor-pointer">
                            <i data-lucide="table" class="w-4 h-4"></i> View Dynamic UI
                        </button>
    ...
    <!-- 👁️ INTERACTIVE DYNAMIC UI VIEWER MODAL -->
    <div x-show="viewModalOpen" class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4" style="display:none;">
        <div @click.away="viewModalOpen = false" class="bg-white rounded-3xl max-w-5xl w-full p-6 shadow-2xl border border-slate-200 space-y-4 max-h-[90vh] flex flex-col">
            
            <!-- Modal Header -->
            <div class="flex items-center justify-between pb-3 border-b border-slate-100 shrink-0">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-2xl bg-emerald-50 text-emerald-700 flex items-center justify-center font-black text-sm">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-emerald-100 text-emerald-800" x-text="selectedSheet ? selectedSheet.category : ''"></span>
                        </div>
                        <p class="text-xs text-slate-500 font-mono" x-text="'Showing ' + filteredRows.length + ' of ' + parsedRows.length + ' records'"></p>
                    </div>
                </div>

                <div class="flex items-center gap-2">
                    <button type="button" @click="viewModalOpen = false" class="p-2 text-slate-400 hover:text-slate-700 rounded-xl hover:bg-slate-100">
            ...
            <div class="flex items-center justify-between gap-3 shrink-0">
                <div class="relative flex-1 max-w-sm">
                    ...
                    <input type="text" x-model="sheetSearch" placeholder="Search any cell in spreadsheet..." class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500">
                </div>

                <template x-if="selectedSheet && selectedSheet.category === 'employees'">
                    <a href="?page=admin-employees" class="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-xl text-xs font-bold transition flex items-center gap-1.5">
            ...
            <div class="overflow-auto border border-slate-200 rounded-2xl flex-1 max-h-[500px]">
                    ...
                    <thead class="bg-slate-900 text-white uppercase text-[10px] font-black sticky top-0 z-20">
                        <tr>
                            ...
                            <template x-for="(col, colIdx) in parsedColumns" :key="colIdx">
                                <th class="py-3 px-3.5 border-r border-slate-800 truncate" x-text="col || ('COL ' + (colIdx + 1))"></th>
                            </template>
                        </tr>
                    </thead>
                        ...
                        <template x-for="(row, rowIdx) in filteredRows" :key="rowIdx">
                            <tr class="hover:bg-emerald-50/50 transition">
                                ...
                                <template x-for="(cell, cellIdx) in row" :key="cellIdx">
                                    <td class="py-2.5 px-3.5 border-r border-slate-100 text-slate-700 font-medium whitespace-nowrap" x-text="cell"></td>
                                </template>
                            </tr>
                        </template>
                    </tbody>
            ...
            <div class="flex items-center justify-between pt-2 border-t border-slate-100 shrink-0 text-xs text-slate-500">
                <span>⚡ Auto-structured via Universal Spreadsheet Parser</span>
                <button type="button" @click="viewModalOpen = false" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl">
                    Close Viewer
                </button>
            </div>
        </div>
