...
onboardModalOpen: false, 
    ...
    termUserEmpId: '',
    termReason: '',
    openTerminateModal(id, name, empId) {
        this.termUserId = id;
        this.termUserName = name;
        this.termUserEmpId = empId;
        this.termReason = '';
                    ...
                    <tr class="hover:bg-slate-50/80 transition <?= $e['role'] === 'admin' ? 'bg-indigo-50/20' : '' ?>">
                        ...
                        <td class="py-3.5 pl-2 pr-5 align-middle text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-1.5">
                                ...
                                <?php if (!empty($e['is_dismissed'])): ?>
                                    <form action="?action=restore-employee" method="POST" class="inline m-0" onsubmit="return confirm('Restore and re-enable login access for <?= addslashes($e['name']) ?>?');">
                                        <input type="hidden" name="user_id" value="<?= (int)$e['id'] ?>">
                                        <button type="submit" class="px-2.5 py-1 text-emerald-700 hover:text-emerald-900 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg transition inline-flex items-center gap-1 text-xs font-bold shadow-2xs cursor-pointer" title="Re-enable / Restore Employee">
                                            <i data-lucide="user-check" class="w-3.5 h-3.5"></i> Restore
                                        </button>
                                    </form>
                                <?php elseif ($e['role'] !== 'admin'): ?>
                                    <button type="button" @click="openTerminateModal(<?= (int)$e['id'] ?>, '<?= addslashes($e['name']) ?>', '<?= addslashes($e['emp_id'] ?? '') ?>')" class="px-2.5 py-1 text-rose-700 hover:text-rose-900 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg transition inline-flex items-center gap-1 text-xs font-bold shadow-2xs cursor-pointer" title="Terminate / Dismiss Employee">
                                        <i data-lucide="user-x" class="w-3.5 h-3.5"></i> Terminate
                                    </button>
                                <?php endif; ?>
                            </div>
                        </td>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="bg-slate-50/70 p-4 rounded-2xl border border-slate-200/80 space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-2.5">
                            <div>
                                <label class="block text-[11px] font-bold text-slate-700 uppercase mb-1">WhatsApp / Phone <span class="text-rose-500">*</span></label>
                                <input type="tel" name="whatsapp_number" required placeholder="9876543210" class="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs font-semibold text-slate-900 focus:ring-2 focus:ring-indigo-500 transition shadow-2xs">
                            </div>
                            <div>
                                <label class="block text-[11px] font-bold text-slate-700 uppercase mb-1">🎂 Date of Birth <span class="text-rose-500">*</span></label>
                                <input type="date" name="date_of_birth" required class="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs font-semibold text-slate-900 focus:ring-2 focus:ring-indigo-500 transition shadow-2xs">
                            </div>
                        </div>
